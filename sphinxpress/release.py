...
def resolve_release_metadata(config: AppConfig, project: ProjectConfig) -> ReleaseMetadata:
...
def resolve_release_tag(config: AppConfig, project: ProjectConfig) -> str:
    ...
    if strategy == "manual":
        if not project.release_tag:
            raise ReleaseResolutionError(
                f"Project '{project.name}' uses manual release metadata but has no release_tag."
            )
    ...
    if strategy == "git_tag":
        ...
        if result.returncode != 0:
            raise ReleaseResolutionError(
                f"Could not resolve git tag for project '{project.name}': "
                f"{result.stderr.strip() or result.stdout.strip() or 'git describe failed'}"
            )
    ...
    if strategy == "pyproject":
        ...
        if not pyproject_path.exists():
            raise ReleaseResolutionError(
                f"Project '{project.name}' is missing pyproject.toml at {pyproject_path}."
            )
        ...
        if not isinstance(version, str) or not version.strip():
            raise ReleaseResolutionError(
                f"Project '{project.name}' does not define [project].version in {pyproject_path}."
            )
...
def build_release_url(config: AppConfig, project: ProjectConfig, tag: str) -> str:
    return config.release.release_url_template.format(repo_url=project.repo_url, tag=tag)
