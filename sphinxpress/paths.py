...
def ensure_within_root(root: Path, candidate: Path) -> Path:
    ...
    try:
        ...
        raise PathTraversalError(
            f"Generated path '{candidate_resolved}' escapes configured root '{root_resolved}'."
        ) from exc
...
def permalink_for(tools_dir: Path, project_name: str, docname: str) -> str:
    ...
    if ".." in parts or not parts:
        raise PathTraversalError(f"Unsafe Sphinx docname '{docname}'.")
    base_parts = [segment.strip("/") for segment in tools_dir.parts if segment not in (".", "")]
...
def generated_notice(comment_prefix: str = "<!--", comment_suffix: str = "-->") -> str:
    return f"{comment_prefix} GENERATED by sphinxpress. Do not edit by hand. {comment_suffix}"
...
def copy_tree(src: Path, dst: Path) -> None:
    ...
    for source_path in sorted(src.rglob("*")):
        ...
        if any(part in {"_build", ".doctrees", "__pycache__"} for part in relative.parts):
            continue
